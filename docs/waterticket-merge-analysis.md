...
3. 현재 저장소의 개인 차단, WebP, 쿼리 최적화, 캐시, 모바일 및 Hios 스킨을 보존한다.
...
Waterticket의 Blade 스킨은 현재 `default` 스킨을 교체하지 않고 `skins/modern` 등 별도 이름으로 가져온다. 관리자가 모듈 설정에서 선택하여 시험할 수 있도록 한다.
...
- Waterticket Blade 스킨을 별도 스킨으로 이식
- 개인 차단, WebP, 현재 업로더 기능 병합
- 하드코딩 문자열 외부화
- 기본, modern, 모바일, Hios 스킨을 병행 유지
...
- 실제 Rhymix 설치 환경에서의 모듈 설치 및 업데이트
- 실제 DB 스키마와 쿼리 실행
- FFmpeg 변환 결과와 브라우저 호환성
- Queue driver별 동작
- 파일 삭제와 롤백 통합 테스트
- 모든 스킨의 브라우저 UI 회귀 테스트

따라서 본 문서는 통합 방향과 위험을 확정하기 위한 분석 문서이며, 각 구현 단계 완료 후 별도의 런타임 검증이 필요하다.
